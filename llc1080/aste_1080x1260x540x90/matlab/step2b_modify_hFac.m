...
%Pacific
  j=2;f4E=compute_gate_transport(obcs2{j}.U,obcs2{j}.dyg,drF',obcs2{j}.hfW);
%GibraltarStrait
  j=4;f1E=compute_gate_transport(obcs2{j}.U,obcs2{j}.dyg,drF',obcs2{j}.hfW);

%computing incoming:
%South Atlantic, positive into domain
  fSA=f1S-f5E;
%Pacific, positive into domain
  fPa=-f4E;
%GibraltarStrait, positive into domain
  fGi=-f1E;

  figure(1);clf;colormap(seismic(21));
  subplot(323);j=3;mypcolor(1:nx,-[1:nz],-mean(obcs2{j}.U,3)');ccp=max(abs(caxis));caxis([-ccp ccp]);
		   mythincolorbar;title('face5, + = northward');
  subplot(321);j=4;mypcolor(1:nfy(1),-[1:nz],-mean(obcs2{j}.U,3)');ccp=max(abs(caxis));caxis([-ccp ccp]);
                   mythincolorbar;title('face1 Gibraltar, +=intoMediter');grid;
                   ii=find(obcs{j}.imask==1);set(gca,'Xlim',[ii(1)-5 ii(end)+5]);% -32 -1]);
  subplot(322);i=1;mypcolor(1:nx,-[1:nz],mean(obcs2{i}.V,3)');ccp=max(abs(caxis));caxis([-ccp ccp]);
		   mythincolorbar;grid;title('face1, + =northward');
  subplot(324);j=2;mypcolor(1:nfy(4),-[1:nz],-mean(obcs2{j}.U,3)');ccp=max(abs(caxis));caxis([-ccp ccp]);
                   mythincolorbar;title('face4 Pacific, +=N');grid;
  nt=size(f1S);nt=nt(2);
  subplot(349);plot(1:nt,fSA,'b-',1:nt,fGi,'g-',1:nt,fPa,'r-');grid;legend('fSA','fGi','fPa');
  %subplot(3,4,10);plot(f15,'b-');hold on;plot(-f4,'r');grid;legend('f15','f4');

  imbl = fSA+fPa+fGi;				%1 x 168
  nt = max(size(imbl));
...
%properly across the faces
  ...
  iobcs1=1;	iobcs5=3;
  iobcs4=2;	iobcsGi=4;
  A1 =drF*obcs2{iobcs1}.dxg; A1=A1'.*obcs2{iobcs1}.hfS;%[1080 x 90]
  A4 =drF*obcs2{iobcs4}.dyg; A4=A4'.*obcs2{iobcs4}.hfW;%[1080 x 90]
  A1E=drF*obcs2{iobcsGi}.dyg;A1E=A1E'.*obcs2{iobcsGi}.hfW;%[1080 x 90]
  A5 =drF*obcs2{iobcs5}.dyg; A5=A5'.*obcs2{iobcs5}.hfW;%[1080 x 90]
  A15=cat(1,A5,A1);			   %[2160 x 90];
%  A15=A1;
  
  dvel=imbl./sum(A15(:));

  obcs2{iobcs1}.dV =  repmat(dvel',[1 nx nz]);obcs2{iobcs1}.dV=permute(obcs2{iobcs1}.dV,[2 3 1]);
  obcs2{iobcs5}.dU = -repmat(dvel',[1 nx,nz]);obcs2{iobcs5}.dU=permute(obcs2{iobcs5}.dU,[2 3 1]);
  obcs2{iobcs4}.dU = zeros(size(obcs2{iobcs4}.U));

%South Atlantic
  f1bl=compute_gate_transport(obcs2{iobcs1}.V-obcs2{iobcs1}.dV,obcs2{iobcs1}.dxg,drF',obcs2{iobcs1}.hfS);
  f5bl=compute_gate_transport(obcs2{iobcs5}.U-obcs2{iobcs5}.dU,obcs2{iobcs5}.dyg,drF',obcs2{iobcs5}.hfW);
%Pacific
  f4bl=compute_gate_transport(obcs2{iobcs4}.U-obcs2{iobcs4}.dU,obcs2{iobcs4}.dyg,drF',obcs2{iobcs4}.hfW);

  fSAbl=f1bl-f5bl;
  resid=fSAbl+(-f4bl)+(fGi);
  subplot(3,4,11);plot(f1bl-f1S,'b-');grid;hold on;plot(-f5bl+f5E,'r-');hold off;legend('f1bl-f1S','f5bl-f5E');
  subplot(3,4,12);plot(resid,'b-');grid;title(num2str(sum(resid)));legend('resid');%,'imbl');hold on;plot(imbl,'r-');
  subplot(323);mypcolor(1:nx,-[1:nz],-mean(obcs2{iobcs5}.U-obcs2{iobcs5}.dU,3)');grid;
 	cc1=max(abs(caxis));caxis([-cc1 cc1]);title('face5bl, + = northward');
  subplot(324);mypcolor(1:nx,-[1:nz],mean(obcs2{iobcs1}.V-obcs2{iobcs1}.dV,3)');
	cc1=max(abs(caxis));caxis([-cc1 cc1]);mythincolorbar;grid;title('face1bl, + =northward');
...
%updating vel to include balancing correction
  ...
  obcs2{iobcs5}.U=obcs2{iobcs5}.U-obcs2{iobcs5}.dU;
  obcs2{iobcs1}.V=obcs2{iobcs1}.V-obcs2{iobcs1}.dV;

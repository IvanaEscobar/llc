...
a=readbin([fBathyIn(1:idot-1) extBathy '.bin'],[nx ny]);[a,af]=get_aste_tracer(a,nfx,nfy);
load([dirOBCS 'step0_obcs_' datestamp '.mat']);	%obcs0 obcs

figure(1);clf;
iface=[1,4,5];
for i=1:length(iface);
  subplot(1,3,i);mypcolor(af{iface(i)}');caxis([-1e2 0]);mythincolorbar;
...
clear type face
figure(1);clf;
for iobcs=[1,size(obcs,2),2,3];%:size(obcs,2)-1];
  fprintf('%s\n',obcs{iobcs}.name);
...
fpr=[dirOBCS 'aste' nxstr 'x' num2str(ncut1) 'x' num2str(ncut2) '_obcsC.png'];print(fpr,'-dpng');fprintf('%s\n',fpr);
...
fpr=[dirOBCS 'aste' nxstr 'x' num2str(ncut1) 'x' num2str(ncut2) '_obcsC270.png'];print(fpr,'-dpng');fprintf('%s\n',fpr);

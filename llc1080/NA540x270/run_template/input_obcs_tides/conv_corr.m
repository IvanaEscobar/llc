...
function [aam,aph] = conv_corr(time,aam,aph,cl)
  % get nodal corrections
  [pu,pf]=nodal(time-datenum(1992,1,1)+48622,cl);
  % loop through constituents
  for i = 1:length(cl)
    % get constituent data
    [ispec,am,ph,omega,alpha,constitNum] = constit(cl(i,:)); % THIS WAS THE BUG!
    ...
    aph(i,:) = (aph(i,:)*pi/180-ph-pu(i))/omega+(datenum(1992,1,1)-time)*86400;
  end
end

...
%ice2wtr=0.8856;snow2wtr=0.3212;             % Not sure what these parameters do when there is no ice

%% Attempt at using regional domain instead of global domain
nx=2160; ny=2160;
...
nfx=[nx 0 0 0 nx/2]; nfy=[nx/2 0 0 0 nx];
...
hf=readbin([dirGrid '../SandSv18p1_NA4320x2160x1080_obcs11Jan2019.bin'],[nx ny]);
...
%eta=readbin([dirIn 'Eta.' ts '.data'],[nx ny]);
...
cclim=[-1,1;-2 12;31 36;0 1]; % Also not sure what this is for
vstr={'etaf'};
figure(4);clf;
...
%  if(strcmp(vstr{ivar},'sstf'));colormap(hh(ivar),seismic(84));end;
  ...
  m_proj('lambert','lat',[26 45],'long',[-85 -5]);
  for i=[1,5];
...
set(gcf,'paperunits','inches','paperposition',[0 0 14 18]);fpr=[dirIn 'eta_mmap.png'];print(fpr,'-dpng');

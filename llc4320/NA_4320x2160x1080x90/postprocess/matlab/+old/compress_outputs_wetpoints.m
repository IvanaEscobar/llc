...
ny= 2160; %id.n.y;
nz= 90;   %id.n.z;
nfx= [ny 0 0 0 ny/2]; %id.nf.x;
nfy= [ny/2 0 0 0 ny]; %id.nf.y;

dirScr='/scratch/05427/iescobar/llc/llc4320/NA_4320x2160x1080x90/';
runStr='run_c67h_pk0000000001';
dirRun=[dirScr runStr '/diags_test/'];
if(~exist(dirRun));error('dirrun not exist');end;

varstrin={'state_3d_hourly','state_3d_hourly','trsp_3d_hourly','trsp_3d_hourly'};
ext     ={'STATE/','STATE/','TRSP/','TRSP/'};
varstrout={'THETA','SALT','UVELMASS','VVELMASS'};
varstr_wetpt={'C','C','W','S'};		%hfac[C,W,S], %,'C'
%sometimes, write file not full depth to nz, but only to nz1:
nzoffset=[0 1 0 1].*nz; % order of variable in .data file
...
for ivar=1:size(varstrin,2);
  ...
  idot=find(flist(1).name=='.');idot=idot(1)+1:idot(2)-1; %idot reads the timestep of the file

%determining nz1 from meta file:
  meta=rdmds_meta([dirRun ext{ivar} flist(1).name(1:end-5)]);
  if(meta.nDims==2);nz1=1;else;nz1=meta.dimList(7);end;			
  ...
  for ifile=istart:length(flist);					%89sec / file
    %tic
    ts=flist(ifile).name(idot); %saving time step
    ...
    if(~exist(fOut));
      ...
      if(use_method==1);
        ...
        for iz=1:nz1;
          clear a ii ioffset
          a=read_slice([dirRun ext{ivar} flist(ifile).name],ny,ny,izv(iz));
          ii=i1(6,iz):i2(6,iz);
          ioffset=(iz-1)*ny*ny;
      ...
      elseif(use_method==0);						%47sec / file
        ...
        ain=read_slice([dirRun ext{ivar} flist(ifile).name],ny,ny,izv);	%21sec to read all 90
...
if(expanding_field==1);							%/*{{{*/
  ...
  for ivar=1:1;
    ...
    for ifile1=1:5;
      ...
      tic;aout=zeros(ny,ny,nz);toc;					%0.02sec
...
%expanding 1 layer:
  ...
  for ivar=1:1;
    ...
    tmp=zeros(ny*ny,1);tmp(iwet)=fld;tmp=reshape(tmp,ny,ny);
  end;

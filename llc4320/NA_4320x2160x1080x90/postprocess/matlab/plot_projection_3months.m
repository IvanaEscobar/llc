%%function plot_projection_NA4320(color_name)
    ...
    dirIn='/scratch/05427/iescobar/llc/llc4320/NA_4320x2160x1080x90/';
    dirRun=[dirIn 'run_c67h_tidal_bc_pk0000000001/'];
    dirGrid=[dirIn 'GRID/'];
    ...
    hf=readbin([dirRun 'SandSv18p1_NA4320x2160x1080_obcs11Jan2019.bin'],[nx ny]);
    hf(find(hf>=0))=nan;hf(find(hf<0))=1;% Depth
...
%    theta=theta.*hf; thetaf=get_aste_faces(theta,nfx,nfy);clear theta
    eta=read_slice([dirRun 'diags/STATE/state_2d_hourly.' ts '.data'], nx, ny, 1); %1st variable in state_2d_hourly*
    ...
    FF=etaf;
    jy{1}=1:1080; ix{1}=1:2160; % Face 1: indicies in local face coordinates for llc gcmfaces
    jy{5}=1:2160; ix{5}=1:1080; % Face 5: j points in v direction, and i in u direction

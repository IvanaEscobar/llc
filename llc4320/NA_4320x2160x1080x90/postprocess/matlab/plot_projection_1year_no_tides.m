%%function plot_projection_NA4320(color_name)
    ...
    dirIn=['/work/05427/iescobar/no_tidal_bc_1year_data/'];
    dirGrid=[dirIn 'GRID/'];
    ...
    ts='0000350640';
    %theta=read_slice([dirIn 'pickup.' ts '.data'], nx,ny, 1+(3-1)*nz);
    %theta=theta.*hf; thetaf=get_aste_faces(theta,nfx,nfy);clear theta
    eta=read_slice([dirIn 'state_2d_daily.' ts '.data'], nx, ny, 1); %1st variable in state_2d_daily*
...
%    end
    ...
    fpr=[dirIn 'eta_z1_' color_name '_no_tidal_bc_ts' ts '.png'];
    print(fpr,'-dpng');
%end
